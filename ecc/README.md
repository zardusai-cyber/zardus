...
ECC ships with **144+ skills** available via **lazy loading** (trigger-based on-demand loading):
...
- api-design
- e2e-testing
- + 130+ more (golang, rust, python, kotlin, docker, database, deployment, etc.)

**Lazy Loading:** Skills load on-demand only when trigger keywords are detected in user messages.
See `~/.config/opencode/agents/skill-triggers.md` for the full trigger table.

**Before:** 12 skills eagerly loaded = ~100KB+ in context
**After:** 0 skills loaded at cold start = ~2KB trigger table only
